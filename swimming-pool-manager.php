...
function ifs_pms_check_db_migrations() { 
    ...
    if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', trim( $t_tick, '`' ) ) ) === trim( $t_tick, '`' ) ) { 
        ...
        if ( is_array( $cols ) ) { 
            ...
            if ( ! in_array( 'room_no', $cols, true ) ) { 
                $wpdb->query( "ALTER TABLE {$t_tick} ADD `room_no` varchar(20) DEFAULT NULL AFTER `payment_method`" ); 
            } 
        } 
    } 
} 
...
/** 
 * 9. Secure CSV Exporter Callback (Managers Only) 
 */ 
...
/** 
 * 11. Replace Powered by WordPress Footer Text 
 */ 
